...
double NewtonianMC::newtmc_int(const double initial_vals,
                               Eigen::VectorXd &draws_out, int iter,
                               int burnin) {
  ...
  for (size_t jj = 0; jj < draws_out.size(); jj++) {
    ...
    comp_val = std::min(0.0, prop_LP - prev_LP + prop->condLogProb(prev_draw) -
